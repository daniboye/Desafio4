...
NOTA DE VERSAO 4: o modelo "llama-3.3-70b-versatile" foi descontinuado
pela Groq. A Groq recomenda a migracao para "openai/gpt-oss-120b" (um
modelo de codigo aberto da OpenAI hospedado na infraestrutura da Groq),
que oferece bom desempenho de raciocinio e suporte a tool-calling. Assim
como aconteceu com o Gemini, provedores de LLM descontinuam nomes de
modelo especificos com regularidade -- ao notar um erro do tipo "model
not found"/"does not exist", verifique a lista de modelos atuais do
provedor (para a Groq, em console.groq.com/docs/models) e atualize a
constante abaixo.
...
# Modelo atual recomendado pela Groq para tarefas gerais de raciocinio e
# tool-calling, apos a descontinuacao do "llama-3.3-70b-versatile".
...
def _criar_llm(provedor: str):
    ...
    de provedor (ou de modelo especifico) no futuro sem mexer no restante
    da montagem do agente.
    """
    if provedor == "groq":
...
def montar_agente(schema_context: str, chave_api: str | None = None) -> AgenteCsv:
    ...
    LLM_PROVIDER. Se nao for definida, o padrao e "groq". O modelo
    especifico da Groq pode ser sobrescrito pela variavel GROQ_MODEL,
    caso o modelo padrao seja descontinuado novamente no futuro.
