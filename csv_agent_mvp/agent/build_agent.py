...
NOTA DE VERSAO 2: usamos o nome de modelo "gemini-flash-latest" em vez de
uma versao especifica como "gemini-2.5-flash". A Google descontinua
periodicamente versoes especificas de modelo para novos usuarios (isso
aconteceu com o gemini-2.5-flash). O "alias" gemini-flash-latest sempre
aponta para a versao Flash mais recente e disponivel no momento, evitando
que o codigo quebre de novo quando a Google atualizar seus modelos.
...
e metadados internos do Google como "extras.signature" -- usados para o
modelo manter memoria do proprio raciocinio entre etapas), em vez de vir
como um texto simples direto. Se nao tratarmos isso, o usuario veria essa
estrutura de dados bruta na tela, em vez de apenas o texto da resposta.
A funcao _extrair_texto_da_resposta(), abaixo, resolve isso.
...
def _extrair_texto_da_resposta(conteudo) -> str:
    ...
    Trata os dois formatos possiveis:
      1) uma string simples (formato mais antigo/comum): devolvida direto.
      2) uma lista de blocos estruturados (formato mais novo do Gemini):
         percorremos a lista e juntamos apenas o texto de cada bloco do
         tipo "text", descartando metadados internos como "extras" (que
         nao tem valor para o usuario final).
    """
